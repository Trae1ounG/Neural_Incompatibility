...
def register_hook(
    ...
    if layer_idx is not None:
      ...
      handles = []
      for layer_idx in range(len(get_attributes(model, transformer_layers_attr))):
        ff_layer = get_ff_layer(
          model,
          layer_idx,
          transformer_layers_attr=transformer_layers_attr,
        ...
        handles.append(ff_layer.register_forward_hook(hook_fn))
      return handles
...
def downsample_bilinear(matrix, target_size=(32, 32)):
    h, w = matrix.shape
    scale_h = target_size[0] / h
    scale_w = target_size[1] / w
    return zoom(matrix, (scale_h, scale_w), order=1)