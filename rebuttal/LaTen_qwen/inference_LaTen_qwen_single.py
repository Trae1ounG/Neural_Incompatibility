...
from modeling_qwen2 import Qwen2ForCausalLM as Qwen2ForCausalLMEdit
...
def fuse_models(
    ...
    # Load models
    teacher_model = Qwen2ForCausalLMEdit.from_pretrained(
        source_model,
        torch_dtype=torch.float32,
        device_map="cuda:0"
    )
    teacher_tokenizer = Qwen2Tokenizer.from_pretrained(source_model)
    
    student_model = Qwen2ForCausalLMEdit.from_pretrained(
        target_model,
        torch_dtype=torch.float32,
        device_map="cuda:0"
    )
    student_tokenizer = Qwen2Tokenizer.from_pretrained(target_model)
    ...
    for step in steps:
        ...
        knowledge_translator.load_state_dict(torch.load(translator_path, map_location="cuda:0"))
        ...
        for transfer_rate in transfer_rates:
            ...
            with torch.no_grad():
                current_knowledge = {k: teacher_knowledge[k].detach().to("cuda:0") 
                                    for k in EXTRACT_MODULE}
                translated_knowledge = knowledge_translator(current_knowledge, top_ffn_layer_idx)
                student_kn.modify_weights_ffn_attn_no_lora_inject(
            ...
            output_path = f"qwen2.5-1.5b-gsm-qwen3b-transfer_rate{transfer_rate * 100}%-step{step}-fix"
